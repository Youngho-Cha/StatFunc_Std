...
#' crucial to check this assumption (e.g., using `cox.zph`) to ensure the
...
#'   itself, the Cox model focuses on the **hazard rate**—the instantaneous risk
...
#'     \item \eqn{h_0(t)} is the **baseline hazard**, which is the hazard when all covariates are zero. It is the non-parametric part of the model.
...
#'   The primary output from a fitted Cox model is the **Hazard Ratio (HR)**. The HR compares the hazard rates of two groups or individuals.
#'
